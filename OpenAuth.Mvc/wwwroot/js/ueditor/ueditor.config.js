 ...
 * 當升級編輯器時，可直接使用舊版配置檔案替換新版配置檔案,不用擔心舊版配置檔案中因缺少新功能所需的參數而導致指令碼報錯。
 **************************提示********************************/

(function () {

    /**
     * 編輯器資原始檔根路徑。它所表示的含義是：以編輯器實例化頁面為當前路徑，指向編輯器資原始檔（即dialog等資料夾）的路徑。
     ...
     * 因此，UEditor提供了針對不同頁面的編輯器可單獨配置的根路徑，具體來說，在需要實例化編輯器的頁面最頂部寫上如下程式碼即可。當然，需要令此處的URL等於對應的配置。
    ...
    window.UEDITOR_CONFIG = {
        ...
        //,textarea:'editorValue' // 提交表單時，伺服器獲取編輯器提交內容的所用的參數，多實例時可以給容器name屬性，會將name給定的值最為每個實例的鍵值，不用每次實例化的時候都設定這個值
        ...
        //,iframeCssUrl: URL + '/themes/iframe.css' //給編輯器內部引入一個css檔案
        ...
        //,readonly : false //編輯器初始化結束后,編輯區域是否是隻讀的，預設是false
        ...
        //,allHtmlEnabled:false //提交到後臺的數據是否包含整個html字串
        ...
        //,'fontfamily':[
        //    { label:'',name:'songti',val:'細明體,SimSun'},
        //    { label:'',name:'kaiti',val:'楷體,標楷體, SimKai'},
        ...
        //右鍵菜單的內容，可以參考plugins/contextmenu.js裡邊的預設菜單的例子，label留空支援國際化，否則以此配置為準
        //,contextMenu:[
        //    {
        //        label:'',       //顯示的名稱
        //        cmdName:'selectall',//執行的command命令，當點選這個右鍵菜單時
        ...
        //,topOffset:30
        //編輯器底部距離工具欄高度(如果參數大於等於編輯器高度，則設定無效)
