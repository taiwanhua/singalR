 ...
 * @see baidu.dom.q
 *
 * @return {HTMLElement|null} 獲取的元素，查詢不到時返回null,如果參數不合法，直接返回參數.
 */
 ...
 * @param {Function} iterator 對每個陣列元素進行呼叫的函式，該函式有兩個參數，第一個為陣列元素，第二個為陣列索引值，function (item, index)。
 * @param {Object} [thisObject] 函式呼叫時的this指針，如果沒有此參數，預設是當前遍歷的陣列
 * @remark
...
/**
 * 對語言層面的封裝，包括型別判斷、模組擴充套件、繼承基類以及對像自定義事件的支援。
 ...
 * 在Browser identification最後新增Version + 數字進行版本標識
 * opera後面的數字保持在9.80不變
 */
...
/**
 * 操作flash對象的方法，包括建立flash對像、獲取flash對像以及判斷flash外掛的版本號
 * @namespace baidu.swf
 */
baidu.swf = baidu.swf || {};
...
/**
 * 瀏覽器支援的flash外掛版本
 * @property version 瀏覽器支援的flash外掛版本
 ...
 * 
 * @param {Object} 	options 					建立flash的選項參數
 ...
 * @param {string} 	options.base 				設定用於解析swf檔案中的所有相對路徑語句的基本目錄或URL
 * @param {string} 	options.bgcolor 			swf檔案的背景色
 * @param {string} 	options.salign 				設定縮放的swf檔案在由width和height設定定義的區域內的位置。允許值：l/r/t/b/tl/tr/bl/br
 * @param {boolean} options.menu 				是否顯示右鍵菜單，允許值：true/false
 * @param {boolean} options.loop 				播放到最後一幀時是否重新播放，允許值： true/false
 * @param {boolean} options.play 				flash是否在瀏覽器載入時就開始播放。允許值：true/false
 * @param {string} 	options.quality 			設定flash播放的畫質，允許值：low/medium/high/autolow/autohigh/best
 * @param {string} 	options.scale 				設定flash內容如何縮放來適應設定的寬高。允許值：showall/noborder/exactfit
 * @param {string} 	options.wmode 				設定flash的顯示模式。允許值：window/opaque/transparent
 * @param {string} 	options.allowscriptaccess 	設定flash與頁面的通訊許可權。允許值：always/never/sameDomain
 * @param {string} 	options.allownetworking 	設定swf檔案中允許使用的網路API。允許值：all/internal/none
 * @param {boolean} options.allowfullscreen 	是否允許flash全屏。允許值：true/false
 * @param {boolean} options.seamlesstabbing 	允許設定執行無縫跳格，從而使使用者能跳出flash應用程式。該參數只能在安裝Flash7及更高版本的Windows中使用。允許值：true/false
 * @param {boolean} options.devicefont 			設定靜態文字對象是否以裝置字型呈現。允許值：true/false
 * @param {boolean} options.swliveconnect 		第一次載入flash時瀏覽器是否應啟動Java。允許值：true/false
 * @param {Object} 	options.vars 				要傳遞給flash的參數，支援JSON或string型別。
 * 
 * @see baidu.swf.create
 * @meta standard
 * @returns {string} flash對象的html字串
 */
 ...
 * @param {Object} 	options.vars 				要傳遞給flash的參數，支援JSON或string型別。
 * 
 * @param {HTMLElement|string} [container] 		flash對象的父容器元素，不傳遞該參數時在當前程式碼位置建立flash對象。
 * @meta standard
...
baidu.flash._Base = (function(){
    ...
    return function(options, callBack){
        ...
        /**
         * 呼叫flash接口的統一入口
         * @param {String} fnName 呼叫的函式名
         * @param {Array} params 傳入的參數組成的陣列,若不許要參數，需傳入空陣列
         * @param {Function} [callBack] 非同步呼叫后將返回值作為參數的呼叫回撥函式，如無返回值，可以不傳入此參數
         * @return {Null}
        */
 ...
 * @param {Object} createOptions 建立flash時需要的參數，請參照baidu.swf.create文件
 * @config {Object} vars 建立imageUploader時所需要的參數
 ...
 * @config {String} vars.uploadDataFieldName POST請求中圖片數據的key,預設值'picdata'
 * @config {String} vars.picDescFieldName POST請求中圖片描述的key,預設值'picDesc'
 * @config {Number} vars.maxSize 檔案的最大體積,單位'MB'
 ...
 * @config {Number} vars.mode mode == 0時，是使用滾動條，mode == 1時，拉伸flash, 預設值為0
 * @see baidu.swf.createHTML
 * @param {String} backgroundUrl 背景圖片路徑
 * @param {String} listBacgroundkUrl 布局控制元件背景
 * @param {String} buttonUrl 按鈕圖片不背景
 * @param {String|Function} selectFileCallback 選擇檔案的回撥
 * @param {String|Function} exceedFileCallback檔案超出限制的最大體積時的回撥
 * @param {String|Function} deleteFileCallback 刪除檔案的回撥
 * @param {String|Function} startUploadCallback 開始上傳某個檔案時的回撥
 * @param {String|Function} uploadCompleteCallback 某個檔案上傳完成的回撥
 * @param {String|Function} uploadErrorCallback 某個檔案上傳失敗的回撥
 * @param {String|Function} allCompleteCallback 全部上傳完成時的回撥
 ...
 * @param {Object} target 目標對像
 * @param {Object} source 源對像
 * @see baidu.array.merge
 * @remark
 * 
1.目標對像中，與源對像key相同的成員將會被覆蓋。<br>
 ...
 * @config {Object} createOptions 建立flash時需要的參數，請參照baidu.swf.create文件
 * @config {String} createOptions.width
 * @config {String} createOptions.height
 * @config {Number} maxNum 最大可選檔案數
...
baidu.flash.fileUploader = baidu.flash.fileUploader || function(options){
     ...
     * @param {String} fieldName 上傳的表單欄位名
...
/**
 * 使用動態script標籤請求伺服器資源，包括由伺服器端的回撥和瀏覽器端的回撥
 * @namespace baidu.sio
 */
baidu.sio = baidu.sio || {};
...
/**
 * 刪除script的屬性，再刪除script標籤，以解決修復記憶體泄漏的問題
 * 
 * @param {HTMLElement} src script節點
 */
...
/**
 * 通過script標籤載入數據，載入完成由瀏覽器端觸發回撥
 * @name baidu.sio.callByBrowser
 * @function
 * @grammar baidu.sio.callByBrowser(url, opt_callback, opt_options)
 * @param {string} url 載入數據的url
 * @param {Function|string} opt_callback 數據載入結束時呼叫的函式或函式名
 * @param {Object} opt_options 其他可選項
 * @config {String} [charset] script的字符集
 * @config {Integer} [timeOut] 超時時間，超過這個時間將不再響應本請求，並觸發onfailure函式
 * @config {Function} [onfailure] timeOut設定后才生效，到達超時時間時觸發本函式
 * @remark
 * 1、與callByServer不同，callback參數只支援Function型別，不支援string。
 * 2、如果請求了一個不存在的頁面，callback函式在IE/opera下也會被呼叫，因此使用者需要在onsuccess函式中判斷數據是否正確載入。
 * @meta standard
...
/**
 * 通過script標籤載入數據，載入完成由伺服器端觸發回撥
 * @name baidu.sio.callByServer
 * @function
 * @grammar baidu.sio.callByServer(url, callback[, opt_options])
 * @param {string} url 載入數據的url.
 * @param {Function|string} callback 伺服器端呼叫的函式或函式名。如果沒有指定本參數，將在URL中尋找options['queryField']做為callback的方法名.
 * @param {Object} opt_options 載入數據時的選項.
 * @config {string} [charset] script的字符集
 * @config {string} [queryField] 伺服器端callback請求欄位名，預設為callback
 * @config {Integer} [timeOut] 超時時間(單位：ms)，超過這個時間將不再響應本請求，並觸發onfailure函式
 * @config {Function} [onfailure] timeOut設定后才生效，到達超時時間時觸發本函式
 * @remark
 * 如果url中已經包含key為「options['queryField']」的query項，將會被替換成callback中參數傳遞或自動產生的函式名。
 * @meta standard
...
baidu.sio.callByServer = /**@function*/function(url, callback, opt_options) {
    ...
    /*
     * 返回一個函式，用於立即（掛在window上）或者超時（掛在setTimeout中）時執行
     */
...
/**
 * 將字串解析成json對象。註：不會自動祛除空格
 * @name baidu.json.parse
 * @function
 * @grammar baidu.json.parse(data)
 * @param {string} source 需要解析的字串
 * @remark
 * 該方法的實現與ecma-262第五版中規定的JSON.parse不同，暫時只支援傳入一個參數。後續會進行功能豐富。
 * @meta standard
 * @see baidu.json.stringify,baidu.json.decode
 *             
 * @returns {JSON} 解析結果json對像
 */
baidu.json.parse = function (data) {
    //2010/12/09：更新至不使用原生parse，不檢測使用者輸入是否正確
...
/**
 * 將字串解析成json對象，為過時接口，今後會被baidu.json.parse代替
 ...
 * @param {JSON} value 需要序列化的json對像
 * @remark
 * 該方法的實現與ecma-262第五版中規定的JSON.stringify不同，暫時只支援傳入一個參數。後續會進行功能豐富。
 * @meta standard
 * @see baidu.json.parse,baidu.json.encode
 *             
 * @returns {string} 序列化后的字串
 */
...
/**
 * 將json對像序列化，為過時接口，今後會被baidu.json.stringify代替
