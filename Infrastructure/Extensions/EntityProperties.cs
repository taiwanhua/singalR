...
namespace Infrastructure.Extensions
{
    public static class EntityProperties
    {
        ...
        /// <param name="properties"> 格式 Expression<Func<entityt, object>> exp = x => new { x.欄位1, x.欄位2 };或x=>x.Name</param>
        /// <returns></returns>
        ...
        /// <param name="lenght">是否包括后欄位具體長度:nvarchar(100)</param>
        /// <returns></returns>
        ...
        /// 
        /// </summary>
        /// <param name="array">將陣列轉換成sql語句</param>
        /// <param name="fieldType">指定FieldType資料庫欄位型別</param>
        ...
        /// <summary>
        ///<param name="sql">要執行的sql語句如：通過EntityToSqlTempName.Temp_Insert0.ToString()字串占位，產生的的sql語句會把EntityToSqlTempName.Temp_Insert0.ToString()替換成產生的sql臨時表數據
        ...
        /// <param name="fieldType">指定產生的陣列值的型別</param>
        /// <param name="sql"></param>
        /// <returns></returns>
        ...
        )
        {
            ...
            {
                //每1000行需要分批寫入(資料庫限制每批至多寫入1000行數據)
        ...
        /// <summary>
        ///此方法適用於數據量少,只有幾列數據，不超過1W行，或幾十列數據不超過1000行的情況下使用
        /// 大批量的數據考慮其他方式
        /// 將datatable產生sql語句，替換datatable作為參數傳入存儲過程
        /// </summary>
        ...
        /// <param name="keyType">true獲取key對應型別,false返回對像Key的名稱</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// specificProperties=null並且validateProperties=null，對所有屬性驗證，只驗證其是否合法，不驗證是否為空(除屬性標識指定了不能為空外)
        /// specificProperties!=null，對指定屬性校驗，並且都必須有值
        /// null並且validateProperties!=null，對指定屬性校驗，不判斷是否有值
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entity"></param>
        /// <param name="specificProperties">驗證指定的屬性，並且非空判斷</param>
        /// <param name="validateProperties">驗證指定屬性，只對欄位合法性判斷，不驗證是否為空</param>
        /// <returns></returns>
            ...
            string[] validateProperties = null)
        {
            ...
            foreach (PropertyInfo propertyInfo in propertyArray)
            {
                ...
                //若存在specificProperties並且屬性為陣列specificProperties中的值，校驗時就需要判斷是否為空
        ...
        /// <summary>
        /// 獲取資料庫型別，不帶長度，如varchar(100),只返回的varchar
        /// </summary>
        /// <param name="propertyInfo"></param>
        /// <returns></returns>
        ...
        /// 驗證資料庫欄位型別與值是否正確，
        /// </summary>
        /// <param name="propertyInfo">propertyInfo為當欄位，當前欄位必須有ColumnAttribute屬性,
        /// 如欄位:標識為資料庫int型別[Column(TypeName="int")]  public int Id { get; set; }
        /// 如果是小數float或Decimal必須對propertyInfo欄位加DisplayFormatAttribute屬性
        /// </param>
        /// <param name="value"></param>
        /// <returns>IEnumerable<(bool, string, object)> bool成否校驗成功,string校驗失敗資訊,object,當前校驗的值</returns>
        ...
        private static readonly Dictionary<Type, string> ProperWithDbType = new Dictionary<Type, string>()
        {
            ...
            {typeof(byte), SqlDbTypeName.Int}, //型別待完
        ...
        /// 驗證資料庫欄位型別與值是否正確，
        /// </summary>
        /// <param name="dbType">資料庫欄位型別(如varchar,nvarchar,decimal,不要帶後面長度如:varchar(50))</param>
        /// <param name="value">值</param>
        /// <param name="propertyInfo">要驗證的類的屬性，若不為null，則會判斷字串的長度是否正確</param>
        /// <returns>(bool, string, object)bool成否校驗成功,string校驗失敗資訊,object,當前校驗的值</returns>
            ...
            PropertyInfo propertyInfo = null)
        {
            ...
            else if (dbType == SqlDbTypeName.Float || dbType == SqlDbTypeName.Decimal || dbType == SqlDbTypeName.Double)
            {
                ...
                //    throw new Exception("請對欄位" + propertyInfo?.Name + "新增DisplayFormat屬性標識");
            ...
            {
                //預設nvarchar(max) 、text 長度不能超過20000
                if (val.Length > 20000)
                {
                    reslutMsg = $"字元長度最多【20000】";
                }
                else
                {
                    int length =
        ...
        /// <param name="expression">指定屬性的值 格式 Expression<Func<entityt, object>> exp = x => new { x.欄位1, x.欄位2 };</param>
        /// <returns>返回的是欄位+value</returns>
        ...
        /// <summary>
        /// 判斷hash的列是否為對應的實體，並且值是否有效
        /// </summary>
        /// <param name="typeinfo"></param>
        /// <param name="dic"></param>
        /// <param name="removeNotContains">移除不存在欄位</param>
        /// <returns></returns>
            ...
            PropertyInfo[] propertyInfo, bool removeNotContains, bool removerKey, string[] ignoreFields = null)
        {
            ...
            foreach (PropertyInfo property in propertyInfo)
            {
                ...
                {
                    //移除主鍵預設為新增數據，將不在編輯列中的有預設值的數據設定為預設值
                    //如果為true預設為新增功能，新增操作所有不能為空的列也必須要提交
        ...
        /// <summary>
        /// 將資料來源對映到新的數據中,目前只支援List<TSource>對映到List<TResult>或TSource對映到TResult
        /// 目前只支援Dictionary或實體型別
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <typeparam name="TResult"></typeparam>
        /// <param name="source"></param>
        /// <param name="resultExpression">只對映返回對象的指定欄位,若為null則預設為全部欄位</param>
        /// <param name="sourceExpression">只對映資料來源對象的指定欄位,若為null則預設為全部欄位</param>
        /// <returns></returns>
        ...
        /// <param name="expression">指定對需要的欄位賦值,格式x=>new {x.Name,x.P},返回的結果只會對Name與P賦值</param>
