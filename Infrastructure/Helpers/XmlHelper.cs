...
/// 可以將此檔案直接編譯成dll，今後程式只需要引用該dll后開頭新增using XmlLibrary;即可。
namespace Infrastructure.Helpers
{
	...
	public class XMLHelper
	{
		...
		/// 新增一個子節點
		/// </summary>
		/// <param name="xDoc">XmlDocument對像</param>
		...
		/// <summary>
		/// 建立一個XML文件，成功建立后操作路徑將直接指向該檔案
		/// </summary>
		/// <param name="fileName">檔案物理路徑名</param>
		...
		/// 新增新節點
		/// </summary>
		/// <param name="parentNode">新節點的父節點對像</param>
		/// <param name="xmlParameter">Xml參數對像</param>
		...
		/// 新增節點屬性
		/// </summary>
		/// <param name="node">節點對像</param>
		/// <param name="namespaceOfPrefix">該節點的名稱空間URI</param>
		...
		public static XmlNode GetNode(string nodeName)
		{
			...
			foreach (XmlNode xns in nlst)  // 遍歷所有子節點
			{
				...
				else
				{
					...
					if (xn != null) return xn;  /// V1.0.0.3新增節點判斷
				}
			}
			return null;
		}
		...
		/// <param name="index">節點索引</param>
		/// <param name="nodeName">節點名稱</param>
		/// <param name="newInnerText">修改後的內容</param>
		...
		/// 修改節點的內容
		/// </summary>
		/// <param name="xmlParameter">XmlParameter對像</param>
		/// <param name="innerText">修改後的內容</param>
		...
		/// 刪除節點
		/// </summary>
		/// <param name="nodeList">需要刪除的節點對像</param>
